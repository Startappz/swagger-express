...
function parseJsDocs(file, fn) {
	fs.readFile(file, function(err, data) {
		...
		for (var i = 0; i < fragments.length; i++) {
			...
			var doc = doctrine.parse(fragment, {
				unwrap : true
			});
...
/**
 * Parse rybyDoc from a ruby file
 * @api    private
 * @param  {String}   file
 * @param  {Function} fn
 */
function parseRubyDocs(file, fn) {
	fs.readFile(file, function(err, data) {
		if (err) {
			fn(err);
		}
		...
		for (var f = 0; f < fragments.length; f++) {
			fragments[f] = fragments[f].replace("=begin", "/**").replace("=end", "**/");
 ...
 * @param  {Function} fn
 */
function readRuby(file, fn) {
	parseRubyDocs(file, function(err, docs) {
...
function generate(opt) {
	...
	opt.fullSwaggerJSONPath = url.parse(/*opt.basePath + */
	opt.swaggerJSON).path;
